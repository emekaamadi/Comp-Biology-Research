% this will run CD_CD123.m
 ...
 parameters = [ 6.022e23, 1e-4, 100, 2.5e5, 25000, 60000, 6000, 4.09e4, 3.1032, 2.32e5, 0.29376, .00023 ];
 suppress_plot = 1;
...
ksteps = 100;
z= zeros(100,5);
kon = logspace(6,1,ksteps);
koff = logspace(-6,-3,ksteps);
for i = 1:length(kon)
    z(i,1) = koff(i);
    z(i,2) = kon(i);
    kd = koff(i)/ kon(i);
...
Z = zeros(100);  
for i =  1:10000
        Z((ceil(i/100)),(mod(i-1,100) +1)) = arr(i,5);
end
 x =[-12 -4];
 y =[-12 -4];
imagesc(x,y,Z)
...
title('Heatmap shows the max dead AML as KCD3 and KCD123 change');
